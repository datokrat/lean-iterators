...
structure Iter {α α'} (m β) [Iterator α α' m β] where
  inner : α

variable {α : Type w} {α' : Type u} {m : Type w → Type w'} {β : Type v}

def Iter.Relations.yielded [Iterator α α' m β] (it it' : Iter (α := α) m β) (output : β) : Prop :=
  Iterator.yielded (m := m) it.inner it'.inner (Iterator.βEquiv.hom output)

def Iter.Relations.skipped [Iterator α α' m β] (it it' : Iter (α := α) m β) : Prop :=
  Iterator.skipped m it.inner it'.inner

def Iter.Relations.done [Iterator α α' m β] (it : Iter (α := α) m β) : Prop :=
  Iterator.done m it.inner

structure Iter.EncodedStep [Iterator α α' m β] (it : Iter (α := α) m β) where
  inner : IterStep.for m it.inner

inductive Iter.Step [Iterator α α' m β] (it : Iter (α := α) m β) where
...
def Iter.EncodedStep.decode [Iterator α α' m β] {it : Iter (α := α) m β} (step : it.EncodedStep) : it.Step :=
...
@[always_inline, inline]
def Iter.EncodedStep.ofInternal [Iterator α α' m β] {it : Iter (α := α) m β} (step : IterStep.for m it.inner) : it.EncodedStep :=
  ⟨step⟩

@[always_inline, inline]
def toIter (m) [Iterator α α' m β] (it : α) : Iter (α := α) m β :=
  ⟨it⟩
...
def Iter.stepH [Monad m]
    [Iterator α α' m β] (it : Iter (α := α) m β) : m it.EncodedStep :=
  Iter.EncodedStep.ofInternal <$> Iterator.step it.inner

@[always_inline, inline]
def Iter.stepCont [Monad m]
    [Iterator α α' m β] (it : Iter (α := α) m β) {γ} : ContT m γ it.Step :=
  fun h => it.stepH >>= (h ·.decode)

@[always_inline, inline]
def Iter.step {α' : Type u} {α β : Type w} (m : Type w → Type w) [Monad m]
    [Iterator α α' m β] (it : Iter (α := α) m β) : m (Iter.Step it) :=
  Iter.EncodedStep.decode <$> it.stepH

@[always_inline, inline]
def Iter.terminationByFinite [Iterator α α' m β] [Finite α m] (it : Iter (α := α) m β) : FiniteIteratorWF α m :=
