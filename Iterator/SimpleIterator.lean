...
variable (m) in
@[always_inline, inline]
def IterationT.step [Iterator α m β] [Monad m] (it : α) : IterationT m (IterStep α β) :=
  { property := Iterator.plausible_step m it
    computation := Iterator.step it }
...
instance [SimpleIterator α m β] [Monad m] : Iterator α m β where
  plausible_step it := SimpleIterator.step it |>.property
  step it := (SimpleIterator.step (α := α) (m := m) it).computation
...
theorem successor_matchStepH {α : Type u} {m : Type w → Type w'} {β : Type v} {γ : Type x} {δ : Type y}
    ...
    (∃ it' b, Iterator.plausible_step m it (.yield it' b) ∧ (IterationT.mapH f <| yield it' b).property x) ∨
    (∃ it', Iterator.plausible_step m it (.skip it') ∧ (IterationT.mapH f <| skip it').property x) ∨
    (Iterator.plausible_step m it .done ∧ (IterationT.mapH f done).property x) := by
...
theorem successor_matchStep {α : Type u} {m : Type w → Type w'} {β : Type v} {γ : Type max u v} {δ : Type y}
    ...
    (∃ it' b, Iterator.plausible_step m it (.yield it' b) ∧ (IterationT.mapH f <| yield it' b).property x) ∨
    (∃ it', Iterator.plausible_step m it (.skip it')  ∧ (IterationT.mapH f <| skip it').property x) ∨
    (Iterator.plausible_step m it .done ∧ (IterationT.mapH f done).property x) :=
...
theorem property_matchStepH {α : Type u} {m : Type w → Type w'} {β : Type v} {γ : Type x}
    ...
    (∃ it' b, Iterator.plausible_step m it (.yield it' b) ∧ (yield it' b).property x) ∨
    (∃ it', Iterator.plausible_step m it (.skip it') ∧ (skip it').property x) ∨
    (Iterator.plausible_step m it .done ∧ done.property x) := by
...
theorem computation_matchStepH {α : Type u} {m : Type w → Type w'} {β : Type v} {γ : Type x}
    ...
    (matchStepH (m := m) (γ := γ) it yield skip done).computation =
      (IterationT.step m it).computation.bindH (match · with
        | ⟨.yield it' out, h'⟩ => (yield it' out).computation.mapH (fun y => ⟨y.1, .yield it' out, y.2, h'⟩)
        | ⟨.skip it', h'⟩ => (skip it').computation.mapH (fun y => ⟨y.1, .skip it', y.2, h'⟩)
        | ⟨.done, h'⟩ => done.computation.mapH (fun y => ⟨y.1, .done, y.2, h'⟩)) := by
