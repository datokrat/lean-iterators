...
@[simp]
theorem Iter.toPureIter_toIterM {α : Type w} {β : Type v} (it : Iter (α := α) β) :
    it.toIterM.toPureIter = it :=
  rfl

@[simp]
theorem Iter.toIterM_toPureIter {α : Type w} {β : Type v} (it : IterM (α := α) Id β) :
    it.toPureIter.toIterM = it :=
  rfl

def Iter.plausible_step {α : Type w} {β : Type v} [Iterator α Id β]
    (it : Iter (α := α) β) (step : IterStep (Iter (α := α) β) β) : Prop :=
  it.toIterM.plausible_step (step.map Iter.toIterM id)

def Iter.Step {α : Type w} {β : Type v} [Iterator α Id β] (it : Iter (α := α) β) :=
  PlausibleIterStep (Iter.plausible_step it)

def Iter.plausible_output {α : Type w} {β : Type v} [Iterator α Id β]
    (it : Iter (α := α) β) (out : β) : Prop :=
  ∃ it', it.plausible_step (.yield it' out)

def Iter.plausible_successor_of {α : Type w} {β : Type v} [Iterator α Id β]
    (it' it : Iter (α := α) β) : Prop :=
  ∃ step, step.successor = some it' ∧ it.plausible_step step

def Iter.plausible_skip_successor_of {α : Type w} {β : Type v} [Iterator α Id β]
    (it' it : Iter (α := α) β) : Prop :=
...
def Iter.finitelyManySteps {α : Type w} {β : Type v} [Iterator α Id β] [Finite α Id]
...
theorem Iter.TerminationMeasures.Finite.rel_of_yield
    {α : Type w} {β : Type v} [Iterator α Id β]
...
def Iter.finitelyManySkips {α : Type w} {β : Type v} [Iterator α Id β] [Productive α Id]
